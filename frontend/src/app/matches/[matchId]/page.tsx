...
export default function MatchThreadPage() {
  ...
  // Armed only for the first-ever review on this swap (not while editing an
  // existing one): if the form is left open and the user navigates away
  // in-app, the unmount cleanup below auto-submits a default 5-star review
  // rather than leaving the swap stuck showing "Item received" forever.
  ...
  useEffect(() => {
    return () => {
      if (pendingAutoReviewRef.current && tokenRef.current) {
        submitReview(tokenRef.current, matchId, {}).catch(() => {
          // Best-effort: the user has already navigated away, so there's
          // nowhere left to surface a failure. They'll see "Item received"
          // again next time they open this match, and can retry then.
        });
      }
    };
  }, [matchId]);
  ...
  function openReviewForm() {
    const editing = swap?.reviewed ?? false;
    setReviewRating(editing ? (swap?.myReviewRating ?? null) : null);
    setReviewComment(editing ? (swap?.myReviewComment ?? "") : "");
    setReviewError(null);
    setIsReviewFormOpen(true);
    // Only a first-time review should auto-submit if abandoned -- editing an
    // already-reviewed swap has a real value to fall back to, so leaving the
    // page should just leave the existing review untouched.
    pendingAutoReviewRef.current = !editing;
  }
  ...
  async function handleSubmitReview(skip: boolean) {
    ...
    try {
      const result = await submitReview(
        token,
        matchId,
        skip ? {} : { rating: reviewRating ?? undefined, comment: reviewComment.trim() || undefined },
      );
      setSwap((prev) =>
        prev
          ? {
            ...
            <div className="flex flex-col gap-3">
              ...
              {!isReviewFormOpen && !swap.reviewed && (
                <button
                  ...
                  className="self-start rounded bg-foreground px-3 py-1.5 text-xs text-background"
                >
                  Item received
                </button>
              )}
                ...
                <div className="flex flex-col gap-2 rounded border border-black/10 p-3 dark:border-white/20">
                  <p className="text-sm">
                    {swap.reviewed
                      ...
                      : `How was your swap with ${match.otherUserName}?`}
                  </p>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    >
                      {swap.reviewed ? "Save" : "Submit review"}
                    </button>
                    {swap.reviewed ? (
                      <button
                        type="button"
                      ...
                      <>
                        ...
                        <span className="text-xs text-zinc-600 dark:text-zinc-400">
                          Skipping — or leaving this page — automatically gives {match.otherUserName} 5 stars.
                        </span>
                      </>
                    )}
                  </div>
