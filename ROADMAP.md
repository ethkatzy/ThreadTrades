...
| 17 | Reviews/ratings | ✅ Done (new `review` table, one row per (swap, reviewer) — `POST /api/matches/{id}/review` only once the swap is `ACCEPTED`; a skipped rating auto-defaults to 5 stars via an `autoGenerated` flag rather than being rejected; ratings are visible pre-match, not just to matched users — `GET /api/users/{id}/reviews`, plus batched `ownerAverageRating`/`otherUserAverageRating` on the swipe deck and matches list; frontend "Item received" button → star-rating form on the match chat page once both sides have accepted, a "Reviews" tab on both the own-profile and matched-user-profile pages, and a standalone `/users/{id}/reviews` page for the pre-match swipe-deck/rating-badge case) | Requested as the first post-v1 feature, once the core loop (through swap accept/reject) was solid. |
